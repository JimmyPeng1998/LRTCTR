function [Xnew,duration,errorOmega,errorGamma]=TR_RGD_RBB2_legacy(X,PA,Omega,SizeOmega,PAGamma,Gamma,SizeGamma,p,opts)
%TR_RGD_RBB2_LEGACY Legacy preconditioned TR-RGD with the RBB2 stepsize.
%
...
if ~isfield( opts, 'err');      opts.err = 1e-8;        end
...
if ~isfield( opts, 'optimizer');  opts.optimizer = 'RGD';  end
...
errorOmega=zeros(opts.maxiter+1,1);
errorOmega(1)=X.error;
...
%         toc
        errorOmegaNow=(PA-Xnew.Px)'*(PA-Xnew.Px);
    else
        t=1;
        %         while t>1e-10
        Xnew.normofCores=0;
...
%                 toc
        ...
        %             if 0.5*errorOmega(epoch-1)/p+X.normofCores-0.5*errorOmegaNow/p-Xnew.normofCores>0.0001*t*GradAndDirec
        %                 break
        %             end
        %
        %             t=t*0.5;
        ...
        %         if abs(errorOmegaNow)<opts.err
        %             disp("The algorithm converges after "+num2str(epoch)+" iterations.")
        %             duration(epoch)=toc;
        %             errorOmega(epoch)=errorOmegaNow;
        %
        ...
        if abs(errorOmegaNow-errorOmega(epoch-1))/errorOmega(epoch-1)<opts.tol
...
%         temp=ComputePx(Xnew,SizeGamma,Gamma);
    ...
    if opts.train_tol > 0 && sqrt(errorOmega(epoch)/(PA'*PA)) < opts.train_tol
        disp("The training residual reaches the tolerance after "+num2str(epoch)+" iterations.")
        break
    end
...
errorOmega=sqrt(errorOmega(1:(epoch))/(PA'*PA));
